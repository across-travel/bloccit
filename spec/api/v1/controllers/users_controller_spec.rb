...
RSpec.describe Api::V1::UsersController, type: :controller do
  ...
  context "unauthenticated users" do
    ...
    #   new_user = build(:user)
    #   put :update, params: {id: my_user.id, user: { username: new_user.username, name: new_user.name, email: new_user.email, password: new_user.password } }
    #   expect(response).to have_http_status(401)
    # end

    it "POST create returns http unauthenticated" do
      new_user = build(:user)
      post :create, params: {user: { username: new_user.username, name: new_user.name, email: new_user.email, password: new_user.password } }
      expect(response).to have_http_status(401)
    end
  end
  ...
  context "authenticated and authorized users" do
    ...
    #     before do
    #       @new_user = build(:user)
    #       put :update, params: { id: my_user.id, user: { username: @new_user.username, name: @new_user.name, email: @new_user.email, password: @new_user.password, role: "admin" } }
    #     end
    ...
    #     before do
    #       put :update, params: {id: my_user.id, user: { username: "", name: "", email: "bademail@", password: "short" }}
    #     end
    ...
    describe "POST create" do
      context "with valid attributes" do
        before do
          @new_user = build(:user)
          post :create, params: { user: { username: @new_user.username, name: @new_user.name, email: @new_user.email, password: @new_user.password, role: "admin" } }
        end
      ...
      context "with invalid attributes" do
        before do
          post :create, params: { user: { username: "", name: "", email: "bademail@", password: "short" } }
        end
