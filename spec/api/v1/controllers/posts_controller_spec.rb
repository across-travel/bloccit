...
RSpec.describe Api::V1::PostsController, type: :controller do
  ...
  context "unauthenticated user" do
    it "PUT update returns http unauthenticated" do
      put :update, params: {topic_id: my_topic.id, id: my_post.id, post: {title: "Post Title", description: "Post Body"}}
      expect(response).to have_http_status(401)
    end

    it "POST create returns http unauthenticated" do
      post :create, params: {topic_id: my_topic.id, post: {title: "Post Title", description: "Post Body"}}
      expect(response).to have_http_status(401)
    end
  ...
  context "authenticated and authorized users" do
    ...
    describe "PUT update" do
      before { put :update, params: {topic_id: my_topic.id, id: my_post.id, post: {title: @new_post.title, body: @new_post.body}} }
      ...
      it "updates a post with the correct attributes" do
        ...
        expect(hashed_json['title']).to eq(updated_post.title)
        expect(hashed_json['body']).to eq(updated_post.body)
      end
    end

    describe "POST create" do
      before { post :create, params: {topic_id: my_topic.id, post: {title: @new_post.title, body: @new_post.body} }}
